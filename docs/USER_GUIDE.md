...
Welcome to **Boop2**, the next-generation text manipulation tool.

## 🚀 Basic Usage
1. **Paste** your text into the editor.
2. Press **`Cmd + B`** (or `Ctrl + B`) to open the Script Palette.
3. **Search** for a script (e.g., "JSON Format", "Base64 Encode").
4. Press **Enter** to apply the transformation.

## 📑 Multi-Tab System
Boop2 allows you to work on multiple documents simultaneously.
- **New Tab:** `Cmd + T` or click the `+` button in the tab bar.
